...
sub _load_xml_from_url {
    ...
    return XML::LibXML->load_xml(
        string => LWP::UserAgent->new->get( $url )->decoded_content
    )->firstChild();
}
...
sub get_checkout_code {
    ...
    for my $i (@{ $info->{items} }) {
        ...
        my $ship = $doc->createElement('shippingCost');
        $ship->appendText($i->shipping) if $i->shipping;
        ...
        $item->appendChild($ship) if $i->ship;
